...
/// Additional packages needed to build blessed on top of SUPPORT_PACKAGES.
const BLESSED_EXTRA_PACKAGES: &[&str] = &[
    "parallel",
    "nullable",
    "arraybuffer-types",
    "js-date",
...
fn build_blessed() {
    ...
    // Collect sources from the extra packages needed for blessed
    let mut sources: Vec<(String, String)> = Vec::new();
    for &pkg in BLESSED_EXTRA_PACKAGES {
        let pkg_src = packages_dir.join(pkg).join("src");
        assert!(
            pkg_src.exists(),
            "Package '{}' not found at: {}",
    ...
    eprintln!(
        "Building blessed ({} modules from {} extra packages)...",
        sources.len(),
        BLESSED_EXTRA_PACKAGES.len()
    );
    ...
    assert!(
      type_errors.is_empty(),
      "blessed: {} modules have type errors:\n{}",
      type_errors.len(),
...
const HYLOGRAPH_SELECTION_EXTRA_PACKAGES: &[&str] = &[
    "lists",
    "ordered-collections",
    "nullable",
    "catenable-lists",
...
#[test]
#[timeout(30000)]
fn build_hylograph_selection() {
    let packages_dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/packages");

    let registry = Arc::clone(&get_support_build().registry);

    let mut sources: Vec<(String, String)> = Vec::new();
    for &pkg in HYLOGRAPH_SELECTION_EXTRA_PACKAGES {
        let pkg_src = packages_dir.join(pkg).join("src");
        assert!(
            pkg_src.exists(),
            "Package '{}' not found at: {}",
    ...
    eprintln!(
        "Building hylograph-selection ({} modules from {} extra packages)...",
        sources.len(),
        HYLOGRAPH_SELECTION_EXTRA_PACKAGES.len()
    );
    ...
    assert!(
        timeouts.is_empty(),
        "hylograph-selection: {} modules timed out:\n{}",
        timeouts.len(),
        timeouts.join("\n")
    );
    ...
    assert!(
        panics.is_empty(),
        "hylograph-selection: modules panicked:\n{}",
        panics.join("\n")
    );

    assert!(
        other_errors.is_empty(),
        "hylograph-selection: build errors:\n{}",
        other_errors.join("\n")
    );
    ...
    assert!(
        type_errors.is_empty(),
        "hylograph-selection: {} modules have type errors:\n{}",
        type_errors.len(),
        type_errors
            .iter()
            .map(|(m, p, e)| format!("{} ({}): {}", m, p.to_string_lossy(), e))
...
fn build_play() {
    ...
    let mut sources: Vec<(String, String)> = Vec::new();
    for &pkg in PLAY_EXTRA_PACKAGES {
        let pkg_src = packages_dir.join(pkg).join("src");
        assert!(
            pkg_src.exists(),
            "Package '{}' not found at: {}",
    ...
    eprintln!(
        "Building play ({} modules from {} extra packages)...",
        sources.len(),
        PLAY_EXTRA_PACKAGES.len()
    );
...
fn build_deku() {
    ...
    let mut sources: Vec<(String, String)> = Vec::new();
    for &pkg in DEKU_EXTRA_PACKAGES {
        let pkg_src = packages_dir.join(pkg).join("src");
        assert!(
            pkg_src.exists(),
            "Package '{}' not found at: {}",
    ...
    eprintln!(
        "Building deku ({} modules from {} extra packages)...",
        sources.len(),
        DEKU_EXTRA_PACKAGES.len()
    );
...
const FUNCTOR1_EXTRA_PACKAGES: &[&str] = &[
    "functor1",
];
...
fn build_functor1() {
    ...
    let mut sources: Vec<(String, String)> = Vec::new();
    for &pkg in FUNCTOR1_EXTRA_PACKAGES {
        let pkg_src = packages_dir.join(pkg).join("src");
        assert!(
            pkg_src.exists(),
            "Package '{}' not found at: {}",
    ...
    eprintln!(
        "Building functor1 ({} modules from {} extra packages)...",
        sources.len(),
        FUNCTOR1_EXTRA_PACKAGES.len()
    );
    ...
    assert!(
        panics.is_empty(),
        "functor1: modules panicked:\n{}",
        panics.join("\n")
    );

    assert!(
        other_errors.is_empty(),
        "functor1: build errors:\n{}",
        other_errors.join("\n")
    );
    ...
    assert!(
        type_errors.is_empty(),
        "functor1: {} modules have type errors:\n{}",
        type_errors.len(),
        type_errors
            .iter()
            .map(|(m, p, e)| format!("{} ({}): {}", m, p.to_string_lossy(), e))
