...
impl ModuleResolvedNames {
    ...
    fn merge_from(&mut self, other: &ModuleResolvedNames) {
...
pub struct ResolutionExports {
    modules: HashMap<Symbol, ModuleResolvedNames>,
}
...
impl ResolvedResult {
    ...
    pub fn lookup_at(&self, offset: usize) -> Option<&ResolvedName> {
        ...
        let idx = self
            .resolutions
            .partition_point(|r| r.src_span.start <= offset);
        if idx == 0 {
            return None;
        }
...
/// Filter a module's names by its explicit export list.
fn filter_by_exports(
    ...
    for export in exports {
        match export {
            ...
            Export::Module(mod_name) => {
                let mod_sym = module_name_to_symbol(mod_name);
...
fn import_prim_module_to_scope(
    module: &crate::cst::ModuleName,
    scope: &mut NameScope,
    qualifier: Option<Symbol>,
...
/// Import all names from a `ModuleResolvedNames` into scope.
fn import_resolved_names_to_scope(
    names: &ModuleResolvedNames,
    scope: &mut NameScope,
    qualifier: Option<Symbol>,
    origin: NameOrigin,
) {
    ...
    for &name in &names.type_operators {
        scope.type_operators.insert(name, origin.clone());
    }
}

/// Import all names from a `ModuleResolvedNames` except those in the hiding list.
fn import_resolved_names_hiding(
    names: &ModuleResolvedNames,
    hidden_items: &[crate::cst::Import],
    scope: &mut NameScope,
    qualifier: Option<Symbol>,
    origin: NameOrigin,
) {
    ...
    for &name in &names.values {
        if !hidden_values.contains(&name) {
            scope
                .values
                .insert(maybe_qualify(name, qualifier), origin.clone());
        }
    ...
    for &name in &names.type_operators {
        if !hidden_type_ops.contains(&name) {
            scope.type_operators.insert(name, origin.clone());
        }
    }
}

/// Import an explicitly named item from the CST import declaration.
/// Does NOT consult any registry — derives scope entries purely from the import syntax.
fn import_explicit_item(
...
fn build_module_scope(module: &Module, resolution_exports: &ResolutionExports) -> NameScope {
    ...
    for import_decl in &module.imports {
        ...
        // Handle Prim and Prim submodules specially (we have their exports built-in)
...
fn walk_literal(r: &mut Resolver, lit: &Literal, locals: &LocalScope, type_vars: &HashSet<Symbol>) {
...
fn walk_type_expr(r: &mut Resolver, ty: &TypeExpr, type_vars: &HashSet<Symbol>) {
    match ty {
        TypeExpr::Var { .. } => {
...
pub fn resolve_names(module: &Module, exports: &ResolutionExports) -> ResolvedResult {
    let scope = build_module_scope(module, exports);
...
mod tests {
    ...
    /// Parse a module and resolve names.
    ...
    fn test_error_undefined_variable() {
        ...
        assert!(
            has_undefined_variable(&result, "unknownVar"),
    ...
    fn test_error_unknown_type_in_signature() {
        let result = resolve("module T where\nx :: UnknownType\nx = 42");
        assert!(
            has_unknown_type(&result, "UnknownType"),
    ...
    fn test_error_unknown_class_in_constraint() {
        ...
        assert!(
            has_unknown_class(&result, "UnknownClass"),
    ...
    fn test_error_unknown_constructor_in_case() {
        let result = resolve("module T where\nf x = case x of\n  UnknownCtor -> 1");
        assert!(
            has_undefined_variable(&result, "UnknownCtor"),
    ...
    fn test_top_level_value_reference() {
        ...
        let f_ref = f_refs
            .iter()
            .find(|r| r.namespace == Namespace::Value)
            .unwrap();
        assert!(
            matches!(f_ref.definition, DefinitionSite::Local(_)),
            "expected local definition for f, got {:?}",
            f_ref.definition
        );
    }
    ...
    fn test_top_level_type_reference_in_sig() {
        ...
        let foo_type_refs: Vec<_> = find_resolutions(&result, "Foo")
            ...
            .collect();
        assert!(
            !foo_type_refs.is_empty(),
            "expected type resolution for 'Foo'"
        );
        assert!(matches!(
            foo_type_refs[0].definition,
    ...
    fn test_top_level_class_reference() {
        ...
        assert!(
            !class_refs.is_empty(),
            "expected class resolution for 'MyShow'"
        );
    ...
    fn test_local_let_binding_reference() {
        ...
        let body_ref = x_refs
            .iter()
            .find(|r| matches!(r.definition, DefinitionSite::LocalVar(_)));
        assert!(
            body_ref.is_some(),
            "expected LocalVar reference for let-bound 'x'"
        );
    }
    ...
    fn test_local_lambda_param_reference() {
        ...
        assert!(result.errors.is_empty());
        let x_refs = find_resolutions(&result, "x");
        let param_ref = x_refs
            .iter()
            .find(|r| matches!(r.definition, DefinitionSite::LocalVar(_)));
        assert!(
            param_ref.is_some(),
            "expected LocalVar reference for lambda param 'x'"
        );
    }
    ...
    fn test_local_case_binder_reference() {
        ...
        assert!(result.errors.is_empty());
        let x_refs = find_resolutions(&result, "x");
        let binder_ref = x_refs
            .iter()
            .find(|r| matches!(r.definition, DefinitionSite::LocalVar(_)));
        assert!(
            binder_ref.is_some(),
            "expected LocalVar reference for case-bound 'x'"
        );
    }
    ...
    fn test_local_function_binder_reference() {
        ...
        assert!(
            binder_ref.is_some(),
            "expected LocalVar reference for function binder 'x'"
        );
    }
    ...
    fn test_local_shadows_top_level() {
        ...
        let x_refs = find_resolutions(&result, "x");
        let local_var_ref = x_refs
            .iter()
            .find(|r| matches!(r.definition, DefinitionSite::LocalVar(_)));
        assert!(
            local_var_ref.is_some(),
            "expected lambda param to shadow top-level 'x'"
        );
    }
    ...
    fn test_imported_value_reference() {
        // Explicit import: bar added to scope from CST
        let result = resolve("module T where\nimport Data.Foo (bar)\nx = bar");
        assert!(
            result.errors.is_empty(),
            "expected no errors, got: {:?}",
        ...
        assert!(
            !bar_refs.is_empty(),
            "expected resolution for imported 'bar'"
        );
        assert!(matches!(
            bar_refs[0].definition,
    ...
    fn test_imported_constructor_reference() {
        ...
        assert!(
            !just_refs.is_empty(),
            "expected resolution for imported 'Just'"
        );
        assert!(matches!(
            just_refs[0].definition,
            DefinitionSite::Imported(_)
        ));
    }

    #[test]
    fn test_imported_explicit_constructors() {
        // Type(Ctor1, Ctor2) import: constructors added explicitly from CST
        let result =
    ...
    fn test_qualified_imported_value() {
        ...
        assert!(
            !qbar_refs.is_empty(),
            "expected resolution for qualified 'Foo.bar'"
        );
        assert!(matches!(
            qbar_refs[0].definition,
            DefinitionSite::Imported(_)
        ));
    }
    ...
    fn test_qualified_imported_type() {
        ...
        assert!(
            !qbar_refs.is_empty(),
            "expected type resolution for 'Foo.Bar'"
        );
        assert!(matches!(
            qbar_refs[0].definition,
            DefinitionSite::Imported(_)
    ...
    fn test_local_type_operator_reference() {
        ...
        assert!(
            !op_refs.is_empty(),
            "expected type operator resolution for '$'"
        );
    ...
    fn test_local_type_alias_overrides_imported_type() {
        // Explicit import adds Codec to types scope, then local alias overrides it
        let result = resolve(
        ...
        let codec_type_refs: Vec<_> = find_resolutions(&result, "Codec")
            ...
            .collect();
        assert!(
            !codec_type_refs.is_empty(),
            "expected type resolution for 'Codec'"
        );
        // The local type alias should override the import
        let has_local = codec_type_refs
            .iter()
            .any(|r| matches!(r.definition, DefinitionSite::Local(_)));
        assert!(
            has_local,
    ...
    fn test_local_data_overrides_imported_type() {
        ...
        assert!(
            bar_type_refs
                .iter()
                .all(|r| matches!(r.definition, DefinitionSite::Local(_))),
    ...
    fn test_lookup_at_finds_correct_resolution() {
        ...
        if let Some(r) = result.resolutions.first() {
            ...
            assert!(
                found.is_some(),
                "expected lookup_at to find resolution at span start"
            );
    ...
    fn test_resolutions_sorted_by_span() {
        ...
        for window in result.resolutions.windows(2) {
            assert!(
                window[0].src_span.start <= window[1].src_span.start,
                "resolutions not sorted: {} > {}",
                window[0].src_span.start,
                window[1].src_span.start
            );
        }
    }
    ...
    fn test_prim_type_resolves_to_prim() {
        ...
        assert!(
            matches!(int_refs[0].definition, DefinitionSite::Prim),
            "expected Prim definition for Int, got {:?}",
            int_refs[0].definition
        );
    }
    ...
    fn test_prim_boolean_type() {
        ...
        assert!(
            !bool_refs.is_empty(),
            "expected type resolution for 'Boolean'"
        );
    ...
    fn test_imported_type_operator() {
        // Explicit type operator import: added to scope from CST
        let result = resolve(
        ...
        assert!(
            matches!(op_refs[0].definition, DefinitionSite::Imported(_)),
            "expected imported definition for '~>', got {:?}",
            op_refs[0].definition
        );
    }
    ...
    fn test_type_annotation_in_expr() {
        ...
        assert!(
            !int_refs.is_empty(),
            "expected type resolution for Int in annotation"
        );
    }
