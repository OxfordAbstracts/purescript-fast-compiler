...
mod tests {
    ...
    fn test_infer_identity_lambda() {
        // \x -> x should be a -> a (same unif var for input and output)
    ...
    fn test_error_occurs_check() {
        // \x -> x x requires x : a and x : a -> b, leading to a = a -> b (infinite)
    ...
    fn test_error_wrong_argument_type() {
        // Lambda expects Boolean (from the if condition), but gets String
