...
pub fn convert_type_expr(ty: &TypeExpr, type_ops: &HashMap<Qualified<TypeOpName>, Qualified<TypeName>>) -> Result<Type, TypeError> {
    match ty {
        ...
        TypeExpr::Forall { vars, ty, .. } => {
            ...
            let all_forall_vars: HashSet<TypeVarName> = vars.iter().map(|(v, _, _)| TypeVarName::new(v.value)).collect();
            let mut bound_in_forall: HashSet<TypeVarName> = HashSet::new();
            ...
            for (v, _visible, kind) in vars {
                ...
                bound_in_forall.insert(TypeVarName::new(v.value));
            }
...
pub fn find_type_scope_conflict(ty: &TypeExpr, conflicts: &HashSet<TypeName>) -> Option<(crate::span::Span, TypeName)> {
    match ty {
...
fn check_forall_kind_ordering(
    ...
    all_forall_vars: &HashSet<TypeVarName>,
) -> Result<(), TypeError> {
    match kind_expr {
        TypeExpr::Var { span, name } => {
            // If the var is declared in this forall but not yet bound (forward reference)
            let tv = TypeVarName::new(name.value);
            if all_forall_vars.contains(&tv) && !bound.contains(&tv) {
