...
pub struct ModuleCheckResult {
    ...
    /// True when this result was restored wholesale from the build-plan
    /// module memo (the module was proven unchanged and `check_one_module`
...
impl ModuleCheckResult {
    /// Build a result for a module restored from its memo: no diagnostics,
    /// carrying the cached generated JS.
    fn from_memo(name: String, js_module_text: Option<String>) -> Self {
        ModuleCheckResult {
            name,
            schemes: Vec::new(),
            ...
            resolved_dicts: Vec::new(),
            inference_error: None,
            decl_outcomes: HashMap::new(),
            hole_diagnostics: Vec::new(),
            validation_errors: Vec::new(),
            kind_errors: Vec::new(),
            coercible_errors: Vec::new(),
            js_module_text,
    ...
    /// Whether this module produced any diagnostic the build reports as an
    /// error (mirrors `build::format_decldb_errors`). Such modules are never
    /// memoized, so they are always re-checked and re-report fresh.
    fn has_errors(&self) -> bool {
        !self.import_errors.is_empty()
            || !self.validation_errors.is_empty()
            || !self.kind_errors.is_empty()
            || !self.coercible_errors.is_empty()
            || !self.exhaustiveness_errors.is_empty()
            || !self.constraint_errors.is_empty()
            || self.inference_error.is_some()
    }
}
...
// ---------------------------------------------------------------------------
// Build-plan module memo
// ---------------------------------------------------------------------------

/// Everything a clean module contributes to a build that can be restored
/// without re-running `check_one_module`: its exported interface plus the
/// per-decl registry hashes and instance keys that downstream modules' SCC
/// dependency resolution reads. The generated JS is deliberately **not**
/// stored — it already lives on disk as `index.js` and is regenerated by a
/// re-check when missing (see the existence guard in the driver loop), so
/// the memo stays small. Only written for modules that produced **no errors**
/// (an errored module is never memoized, so it is always re-checked and
/// re-reports its diagnostics).
...
struct ModuleMemo {
    ...
    /// `(class_name, instance_decl_key)` pairs to rebuild both instance maps.
    instances: Vec<(String, String)>,
}

/// Direct in-build user-module imports of each module (index → indices),
/// mirroring the edge extraction in [`topo_sort_modules`]. Prim and
/// not-in-this-build imports are skipped — they cannot change within a run.
fn direct_import_indices(
    modules: &[ModuleInput],
    name_index: &HashMap<String, usize>,
) -> Vec<Vec<usize>> {
    let mut out: Vec<Vec<usize>> = vec![Vec::new(); modules.len()];
    for (i, input) in modules.iter().enumerate() {
        for imp in &input.module.imports {
            let imported = imp
                .module
                .parts
                .iter()
...
/// Compute the build plan: `dirty[i] == true` means module `i` must be
/// re-checked; `false` means it is unchanged and can be skipped/restored.
///
/// Coarse rule (transitively correct via topo order): a module is dirty iff
...
/// The transitive import-closure of the dirty set: every clean module that a
/// dirty module (transitively) depends on. Only these clean modules need to
/// be restored into the registry — a clean module that nothing dirty imports
/// is never referenced this build, so it can be skipped entirely (its
/// `index.js` already sits on disk). This is what turns a no-op rebuild from
/// "restore all N modules" into "restore only what the few dirty ones need".
fn compute_needed(dirty: &[bool], imports_idx: &[Vec<usize>]) -> Vec<bool> {
    let n = dirty.len();
    let mut needed = vec![false; n];
    let mut stack: Vec<usize> = Vec::new();
    for i in 0..n {
        if dirty[i] {
            stack.extend(imports_idx[i].iter().copied());
        }
    }
    while let Some(j) = stack.pop() {
        if !needed[j] {
            needed[j] = true;
            stack.extend(imports_idx[j].iter().copied());
        }
    }
    needed
}
...
fn restore_from_memo(
    db: &TypecheckDb,
    registry: &mut ModuleRegistry,
    name: &str,
) -> Option<ModuleCheckResult> {
    ...
    registry.restore_module(
        name,
        memo.exports,
        memo.scheme_hashes,
        memo.nonvalue_hashes,
        memo.instances,
    );
    Some(ModuleCheckResult::from_memo(name.to_string(), None))
}
...
fn write_module_memo(
    ...
    let memo = ModuleMemo {
        ...
        instances: registry.instances_with_class_for(name),
    };
...
fn check_many_modules_inner(
    ...
    // Build plan: decide which modules can be restored from their memo
    // instead of re-checked. Only on a persistent db (the memo is useless
    // for a throwaway in-memory run, and `--no-cache` must reflect a true
    // cold build).
    //
            ...
            if !source_changed && !deps_stale {
                ...
                if output_present {
                    if needed[*idx] {
                        ...
                        // restore its exports + hashes into the registry.
                        if let Some(result) =
                        ...
                        // Memo missing/corrupt → fall through to re-check.
                    } else {
                        // Nothing being re-checked depends on it: skip entirely.
                        n_skipped += 1;
                        on_result(ModuleCheckResult::from_memo(
                            input.name.clone(),
                            None,
                        ));
                        continue;
                ...
                // else: output missing → fall through to regenerate it.
            }
        }
    ...
    if plan_enabled {
        eprintln!(
            "Build plan: {} checked, {} restored from memo, {} unchanged & skipped",
            n_checked, n_restored, n_skipped,
        );
    }
...
fn check_one_module(
    ...
    macro_rules! phase {
        ($label:expr, $body:expr) => {{
            // Feeds the process-wide aggregate table (TYPECHECK_DB_TIMING);
            // `Scope::new` is near-zero overhead when timing is disabled.
            let _scope = phase_timing::Scope::new($label);
    ...
    // Aggregate SCC-loop timing into the process-wide phase table so a
    // `TYPECHECK_DB_TIMING=1` run shows where the value-inference loop spends
    // its time (dep-resolve walk vs cache lookup vs fresh inference).
    if phase_timing::enabled() {
        phase_timing::record("4.scc_dep_resolve", scc_dep_resolve_total);
        phase_timing::record("4.scc_cache_lookup", scc_post_total);
        let scc_inference = scc_full_iter_total
            .saturating_sub(scc_dep_resolve_total)
            .saturating_sub(scc_post_total);
        phase_timing::record("4.scc_inference_misc", scc_inference);
    }
    ...
    let (js_module_text, codegen_outcomes) = if db.codegen_enabled() {
        let _cg_scope = phase_timing::Scope::new("7.codegen");
