...
/// Parse + check a single (import-free) module via the IDE entry point, which
/// enables span→type recording. Span-recording is off on the normal build
/// path (perf), so hover tests must go through `check_module_ide`.
fn check_one_ide(name: &str, src: &str) -> ModuleCheckResult {
    ...
    check_module_ide(&mut db, &ModuleInput::new(name, src, module), &mut registry)
}
