...
use crate::typecheck_db::driver_multi::{
    check_module_ide, check_many_modules_with_db, ModuleInput,
};
...
impl Backend {
    pub(crate) async fn load_sources(&self) {
        ...
        // Phase A: Try to restore the CST-derived LSP snapshots from disk (fast
        // path for goto-def / completion). The registry is NOT restored here —
        // it is rebuilt by the full build below (the persistent SQLite
        // TypecheckDb makes warm restarts cheap), which hover/diagnostics need.
        if let Some(ref cache_dir) = self.cache_dir {
            ...
            if let (Ok(idx), Ok(re), Ok(mfmap), Ok(comp)) =
                (idx_result, re_result, mfmap_result, comp_result)
            {
                {
                ...
                self.info(format!(
                    "[timing] Phase A complete (snapshots loaded): {:.2?}",
                    phase_a_start.elapsed()
                ))
                .await;
            } else {
                self.info(format!(
                    "[timing] Phase A incomplete (missing snapshots): {:.2?}",
                    phase_a_start.elapsed()
                ))
                .await;
            }
        ...
        // Phase B: always run the full build to populate the registry (and
        // refresh the snapshots). The persistent TypecheckDb + its SQLite cache
        // keep warm restarts fast.
        self.spawn_full_build(cmd).await;
        let _ = total_start;
    }

    /// Full build: run the sources command, discover + parse every `.purs`
    /// file, build the CST-derived indexes, and populate the registry via a
    /// persistent `TypecheckDb` build (`check_many_modules_with_db`).
    async fn spawn_full_build(&self, cmd: String) {
        ...
        std::thread::Builder::new()
            ...
            let parsed: Vec<(String, String, cst::Module)> = parse_pool.install(|| {
                sources
                    .par_iter()
            ...
            // Full build with typechecking via the persistent TypecheckDb.
            let t = std::time::Instant::now();
            let inputs: Vec<ModuleInput> = parsed
                .iter()
                .map(|(_path, source, module)| {
                    let name = format!("{}", module.name.value);
                    ModuleInput::new(name, source.clone(), module.clone())
                })
                .collect();

            let report = {
                let mut db_guard = rt.block_on(async { db.lock().await });
                check_many_modules_with_db(&mut db_guard, inputs)
            };
            let error_count: usize = report.results.iter().filter(|r| has_errors(r)).count();
            info(format!("[timing] full build: {:.2?}", t.elapsed()));

            // Build indexes (CST-derived)
            let t = std::time::Instant::now();
                ...
                client
                    .send_notification::<notification::Progress>(ProgressParams {
                        ...
                        value: ProgressParamsValue::WorkDone(WorkDoneProgress::End(
                            WorkDoneProgressEnd {
                                message: Some(format!(
                                    "Loaded {module_count} modules ({error_count} with errors)"
                                )),
                            },
                        )),
                    })
            ...
            typecheck_open_files(&files, &registry, &db, &client, &rt);
        })
        .expect("failed to spawn load-sources thread");
    }
}

/// Whether a module result carries any error diagnostic.
fn has_errors(r: &crate::typecheck_db::driver_multi::ModuleCheckResult) -> bool {
    !r.import_errors.is_empty()
        || !r.validation_errors.is_empty()
        || !r.kind_errors.is_empty()
        || !r.coercible_errors.is_empty()
        || !r.exhaustiveness_errors.is_empty()
...
fn typecheck_open_files(
    ...
    db: &Arc<tokio::sync::Mutex<TypecheckDb>>,
    client: &Client,
    ...
    for (uri_str, source) in open_files {
        ...
        rt.block_on(async {
            ...
            let check_result = {
                ...
                check_module_ide(&mut db_guard, &input, &mut reg)
            };

            let diagnostics = super::diagnostics::to_diagnostics(&check_result, &source);
